...
We simply view the data in a table, and view the largest values for each column. We first notice that there is a wine observation with a residual sugar value of 65.80, more than double that of the next wine observation.
...
We also notice that there is an unusual observation in the free sulfur dioxide column with a value of 289.0, almost double that of the next observation.
...
We performed an 80/20 test/train split on the data so that we can later check how our model would perform on unseen data, and to ensure that our model doesn't overfit to our data.
...
We perform final checks on our final model to ensure that it obeyed the linear model assumptions.
